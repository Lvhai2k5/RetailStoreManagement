...
public class DefectiveQueryRepository {
        ...
        String sql = """
                SELECT
                    ...
                    SUM(CASE WHEN b.ExpiryDate IS NOT NULL
                              AND DATE(b.ExpiryDate) BETWEEN CURRENT_DATE() AND DATE_ADD(CURRENT_DATE(), INTERVAL 7 DAY)
    ...
    public List<Map<String, Object>> searchBatchDefectiveData(LocalDate fromDate,
        ...
        String sql = """
                ...
                WHERE (:fromDate IS NULL OR DATE(COALESCE(
                            (SELECT MAX(d.CreatedDate) FROM DefectiveProducts d WHERE d.BatchID = b.BatchID),
                            b.ImportDate
                        )) >= :fromDate)
                  AND (:toDate IS NULL OR DATE(COALESCE(
                            (SELECT MAX(d.CreatedDate) FROM DefectiveProducts d WHERE d.BatchID = b.BatchID),
                            b.ImportDate
                        )) <= :toDate)
                  AND (:keyword IS NULL OR :keyword = ''
                       OR CAST(b.BatchID AS CHAR(30)) LIKE CONCAT('%', :keyword, '%')
